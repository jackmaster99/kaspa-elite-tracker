...
txt = {
    "Português": {
        ...
        "erro_api": "⚠️ Erro de conexão com a API. Preços em 0.00."
    },
    "English": {
        "titulo": "KASPA ELITE TRACKER",
        ...
        "erro_api": "⚠️ API Connection Error. Prices at 0.00."
    }
}
...
def buscar_dados():
    try:
        k_req = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=kaspa&vs_currencies=usd,brl&include_24hr_change=true", timeout=5)
        k = k_req.json()['kaspa']
        d_req = requests.get("https://api.exchangerate-api.com/v4/latest/USD", timeout=5)
        d = d_req.json()['rates']['BRL']
        return k, d
    except: 
        return {"usd": 0.0001, "brl": 0.0001, "usd_24h_change": 0.00}, 5.2500
...
if wallet_address:
    try:
        ...
        balance_usd = balance_kas * (kas['usd'] if kas['usd'] > 0 else 0)
...
# 2. CALCULADORA (BLINDADA CONTRA DIVISÃO POR ZERO)
st.markdown('<div class="card">', unsafe_allow_html=True)
st.subheader(txt[lang]["calc_titulo"])
ca, cb, cc = st.columns(3)
v_usd = ca.number_input("Dólar ($):", value=1000.0, step=0.01, format="%.4f")

# Trava de Segurança: só calcula se o preço for maior que zero
if kas['usd'] > 0:
    cb.write(f"↳ **{v_usd / kas['usd']:.4f} KAS**")
    cc.write(f"↳ **R$ {v_usd * dolar_real:.4f}**")
else:
    st.error(txt[lang]["erro_api"])
...
for b in baleias:
    ...
    <div style="border:1px solid #FFD700;padding:15px;border-radius:10px;margin-bottom:10px;background-color:#0a0a00;">
        ...
        <a href="https://kas.fyi/address/{b['wallet']}" target="_blank" style="color:#00FF7F;">Verify on Blockchain 🔗</a>
    </div>
